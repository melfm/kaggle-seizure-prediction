...
"""
Generic dataset class that handles data batch processing
as well as splitting the data into three categories :
    training_set, validation_set, test_set if available
"""
...
def train_and_validate(ds, instance):
    ...
    with tf.Graph().as_default():
        ...
        store_predictions(ds, instance, predictions)
        return predictions


def store_predictions(ds, instance, predictions):
    ...
    frame.to_csv(FLAGS.model_dir + str(FLAGS.patient_id) + '_' +
                 str(instance) + '_res.csv', index=False)
    print('Saved results in: ', FLAGS.test_set)
    return predictions


def store_total_avg_pred(ds, instances_count, predictions):
    frame = pd.DataFrame({'File': ds.test.labels.tolist(),
...
if __name__ == '__main__':
    ...
    user_dir = '/home/' + username
    for i in xrange(1, 3):
        exp_model_dir = user_dir + \
            '/seizure_models/single_side_fft/patient_{0}/'.format(i)
    ...
    parser.add_argument(
        '--model_dir', type=str, default=user_dir +
        '/seizure_models/single_side_fft/patient_{0}/'.format(patient_id),
        help='Directory for storing data')
    parser.add_argument(
        '--train_set', type=str,
        default='image_train_{0}_1000/single_side_fft/'.format(patient_id),
        help='Directory for storing data')

    parser.add_argument(
        '--test_set', type=str,
        default='image_test_{0}_1000/single_side_fft/'.format(patient_id),
        help='Directory for storing data')
    ...
    parser.add_argument(
        '--max_localmin_iters', type=int, default=100,
